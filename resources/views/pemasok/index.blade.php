<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 container mx-auto py-4">
        <!-- Breadcrumb -->
        <div class="text-zinc-400 text-xs mb-6">Home > Kelola data > Pemasok</div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div
                ...
                <form method="GET" class="flex flex-wrap items-center gap-2">
                    <div class="relative">
                        <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Cari Pemasok"
                    ...
                    <input type="hidden" name="sort" value="{{ $sort ?? 'nama_pemasok' }}">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($items as $i => $row)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 font-medium text-slate-700">{{ $row->kode_pemasok }}</td>
                                <td class="px-4 py-3 text-slate-700">{{ $row->nama_pemasok }}</td>
                                <td class="px-4 py-3 text-center tabular-nums">{{ $row->telepon_pemasok }}</td>
                                <td class="px-4 py-3 text-center">{{ Str::limit($row->alamat_pemasok, 40) }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="inline-flex gap-2">
                                        <a href="{{ route('pemasok.edit', $row->id_pemasok) }}"
                                            class="px-2 py-1 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50">Edit</a>
                                        <form action="{{ route('pemasok.destroy', $row->id_pemasok) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-slate-500">
                                    Belum ada data. <a href="{{ route('pemasok.create') }}"
